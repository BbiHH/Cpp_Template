...
void update_exc(int l,int r,int c,int s,int t,int p)
{
	...
	if(l<=m) update_exc(l,r,c,s,m,p*2);
	if(r> m) update_exc(l,r,c,m+1,t,p*2+1);
...
int getsum_exc(int l,int r,int s,int t,int p)
{
	...
	//左儿子与查询区间有交集,递归查询左儿子
	if(l<=m) sum+=getsum_exc(l,r,s,m,p*2);
	//右儿子与查询区间头交集,递归查询右儿子
	if(r> m) sum+=getsum_exc(l,r,m+1,t,p*2+1);
...
int main()
{
	...
	scanf("%d",&n);
	build(1,n,1);
	update_exc(1,n,1,1,n,1);
	int q,x,y,z;
	scanf("%d",&q);
	while(q--)
	{
		scanf("%d%d%d",&x,&y,&z);
		update_exc(x,y,z,1,n,1);
	}
	printf("Case %d: The total value of the hook is %d.",o,getsum_exc(1,n,1,n,1));
	}
	return 0;
}
